...
<%!String previtmcode;int prevqty; %>
<%!int n=0; %>
<%! String[] itmcode_m=new String[20];int i;int pos;int k;int l;;
int[] qty_m=new int[20]; %>
<%
  ...
  String heading = null;
    ...
    pos=-1;
    //out.println("pos :"+request.getParameter("pos"));
	if(request.getParameter("pos") != null)
	pos=Integer.parseInt(request.getParameter("pos"));
	 i=(Integer)session.getAttribute("i");
     itmcode_m=(String[])session.getAttribute("array_code");
     qty_m=(int[])session.getAttribute("array_qty");
	   //for(int j=0;j<i;j++){
		 //  out.println(array[j]);
	  // }
	   if(pos >0){
	   ...
	   for(k=pos;k<=i;k++)
	   {
	    itmcode_m[k]=itmcode_m[k+1];
	    qty_m[k]=qty_m[k+1];
	   }
	   i=i-1;
	 for(int j=0;j<(i);j++){
	  // out.println("itm:"+itmcode_m[j]+"i val"+i);
	  }

	  session.setAttribute("array_code",itmcode_m);
	  session.setAttribute("array_qty",qty_m);
	  session.setAttribute("i", i);
	  if((itmcode_m[0]==(null))&&(itmcode_m[1]==(null))){
		    RequestDispatcher view=request.getRequestDispatcher("/counter3.jsp");
  ...
  session.setAttribute("itmcode",strm);
  session.setAttribute("qty",strn);
 // out.println(strn +"   "+strm+"  ");
  if(strn != null && strm != null){

		  itmcode_m[i]=(String)session.getAttribute("itmcode");
   	      qty_m[i]=Integer.parseInt((String)session.getAttribute("qty"));
   	      previtmcode=itmcode_m[i];
   	      prevqty=qty_m[i];
		 // out.println(itmcode_m[i]+"  "+qty_m[i]+"  "+i);
		  i = new Integer(i + 1);
		  session.setAttribute("array_code",itmcode_m);
		  session.setAttribute("array_qty",qty_m);
		  session.setAttribute("i", i);
    ...
    <div class="row">
      ...
      <!-- <p><span class="glyphicon glyphicon-phone"></span>Phone: +00 1515151515</p> -->
       ...
       <div class="col-sm-4">
              ...
              <button class="btn btn-block" data-toggle="modal" data-target="#cartModal"><b>View Cart</b>
    ...
    <div class="row text-center">
      <div class="col-sm-4">
        <div class="thumbnail">
          ...
          <form name="itmform" role="form"  method="post" action="counter2.jsp" >
  ...
  <div class="modal fade" id="cartModal" role="dialog">
    <div class="modal-dialog ">
      ...
      <div class="modal-content">
        ...
        <%!String itmname;int itmcost; int total;int subtotal; %>
        <%
        ...
        <tbody>
         ...
         if(itmcode_m[0]!= null){
           ...
           for(int j=0;j<i;j++){
        	   ...
        	   out.println("<td style=\"text-align:right;\">");
        	   out.println("<i class=\"fa fa-rupee\" style=\"font-size:15px;\"></i> "+itmcost*qty_m[j]);
        	   out.println("</td>");

        	   out.println("<td>");
        	   out.println("<form method=\"post\" action=\"counter2.jsp\" >");
        	   out.println("<input type=\"hidden\" name=\"pos\" value=\""+(j+1)+"\">");
        	   out.println("<button type=\"submit\" >�</button>");
        	   out.println("</form>");
        	   out.println("</td>");
        	   out.println("</tr>");
        	   subtotal=subtotal+(itmcost*qty_m[j]);
           }
         }
         ...
         <button type="submit"  class="btn btn-block">SUBMIT - ORDER
                <span class="glyphicon glyphicon-ok"></span>
              </button>
         </form>
        </div>
        ...
        <div class="row">
        <div class="col-sm-4">
        <form method="post" action="counter2.jsp">
        <input type="hidden" name="pos" value="-2"/>
          <button type="submit" class="btn btn-danger btn-default pull-left" >
            <span class="glyphicon glyphicon-remove"></span> Check-Out
          </button>
          </form>
        </div>
        <div class="col-sm-8">
          <%
           out.println("<p Style=\"font-size:30px;\">Sub-Total : <i class=\"fa fa-rupee\" style=\"font-size:30px;\"></i> "+" "+subtotal+"</p>");
          %>
          </div>
...
<div id="feedback" class="container">
  ...
  <p class="text-center"><em>We love to help our Coustmer!</em></p>
